...
class CNN_Transformer(nn.Module):
    ...
    def forward(self, x, x_mask=None, return_pretransformer=False):   
        b, k, d = x.shape
        mask = (x[:,:,0] != 0)
        raw_valid_counts = mask.sum(dim=1)
        x = self.IN(x,mask)
        ...
        x = x.view((b, d, -1))
        x = torch.transpose(x, 1, 2)
        # This is the CNN output in the original token space, before
        # positional encoding and before the Transformer.  It is exposed only
        # for the Experiment 2B dynamic descriptor; the default stable path
        # below is byte-for-byte the original implementation.
        pre_valid_counts = ((raw_valid_counts - int(self.conv1.kernel_size[0]))
                            // int(self.conv1.stride[0]) + 1).clamp(
                                min=0, max=x.shape[1])
        pre_positions = torch.arange(x.shape[1], device=x.device).unsqueeze(0)
        pre_valid_mask = pre_positions < pre_valid_counts.unsqueeze(1)
        pre_tokens = x * pre_valid_mask.unsqueeze(-1).to(x.dtype)
        last_vals = x[:, -1:, :]
        matches = (x == last_vals)
        ...
        if return_pretransformer:
            return x, pre_tokens.transpose(1, 2), pre_valid_mask
        return x
...
class VarCoNet(nn.Module):
    ...
    def forward_with_pretransformer(self, x):
        """Return the unchanged stable learned-FC plus pre-Transformer tokens."""
        stable_latent, pre_tokens, pre_valid_mask = self.extract(
            x, return_pretransformer=True
        )
        stable = upper_triangular_cosine_similarity(stable_latent)
        return {
            "stable": stable,
            "pretransformer": pre_tokens,
            "pre_valid_mask": pre_valid_mask,
        }
