...
class CNN_Transformer(nn.Module):
    ...
    def forward(self, x, x_mask=None, return_mask=False):   
        b, k, d = x.shape
        mask = (x[:,:,0] != 0)
        # Preserve the raw contiguous padding length before the learned
        # convolution.  The original output-mask heuristic below is retained
        # unchanged for encoder computation, but can occasionally contain an
        # interior equality to the final padded convolution value.  Dynamic
        # windows require the true contiguous temporal support instead.
        raw_valid_counts = mask.sum(dim=1) if return_mask else None
        x = self.IN(x,mask)
        ...
        x = torch.transpose(x, 1, 2)
        if return_mask:
            kernel = int(self.conv1.kernel_size[0])
            stride = int(self.conv1.stride[0])
            latent_valid_counts = torch.div(
                raw_valid_counts - kernel,
                stride,
                rounding_mode='floor',
            ) + 1
            latent_valid_counts = latent_valid_counts.clamp(
                min=0,
                max=x.shape[-1],
            )
            ...
            # Assign this only after all original x_mask uses above, so the
            # default VarCoNet forward and its stable learned-FC are exactly
            # unchanged.  It makes the returned validity mask contiguous.
            x_mask = positions >= latent_valid_counts.unsqueeze(1)
            # The transformer mask is True for padding; expose the inverse
            # alongside the latent [B, R, L] tensor for dynamic features.
            return x, ~x_mask
        return x
...
class VarCoNet(nn.Module):
    ...
    def forward_with_latent(self, x):
        """Return the original stable connectome and its latent token map."""
        h, valid_mask = self.extract(x, return_mask=True)
        stable = upper_triangular_cosine_similarity(h)
        return {
            "stable": stable,
            "latent": h,
