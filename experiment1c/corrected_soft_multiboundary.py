"""Corrected control-referenced soft multi-boundary readout for Experiment 1B-R.
...
def diagnostics_from_model(
    ...
    """Summarize selected-boundary behavior separately for ASD and HC.

    The previous Experiment 1B pooled responsibilities across both diagnoses.
    That pooled 50/50 quantity is not an ASD subtype occupancy estimate.  The
    corrected experiment therefore records per-diagnosis mass and entropy
    only on the unique reference set, which is never used for validation or
    test selection.
    """
    model.eval()
    with torch.no_grad():
        ...
        asd = labels == 0
        hc = labels == 1
        if int(asd.sum().item()) == 0 or int(hc.sum().item()) == 0:
            raise RuntimeError("CRSM diagnostics require ASD and HC reference subjects")
    ...
    return {
        "crsm_asd_mass1": float(q[asd, 0].mean().cpu()),
        "crsm_asd_mass2": float(q[asd, 1].mean().cpu()),
        "crsm_asd_entropy": float(entropy[asd].mean().cpu()),
        "crsm_hc_mass1": float(q[hc, 0].mean().cpu()),
        "crsm_hc_mass2": float(q[hc, 1].mean().cpu()),
        "crsm_hc_entropy": float(entropy[hc].mean().cpu()),
        "crsm_delta1_l2": float(norms[0].cpu()),
        "crsm_delta2_l2": float(norms[1].cpu()),
        "crsm_delta_cosine": float(cosine.cpu()),
        "crsm_delta_bias1": float(model.delta_bias[0].detach().cpu()),
        "crsm_delta_bias2": float(model.delta_bias[1].detach().cpu()),
...
def is_exact_zero_state(state: dict[str, Any]) -> bool:
    """Return true only for bitwise zero CRSM residual parameters."""
    return bool(
        torch.equal(state["delta_weight"], torch.zeros_like(state["delta_weight"]))
        and torch.equal(state["delta_bias"], torch.zeros_like(state["delta_bias"]))
    )
