...
EXP_DIR="/data/adodas/VarCoNet-V2-main/xiaolunwen/experiment3"
...
if [[ ! -f "$EXP_DIR/results/.smoke_pass.json" ]]; then
  {
    ...
    CUDA_VISIBLE_DEVICES=0 OMP_NUM_THREADS=6 MKL_NUM_THREADS=6 OPENBLAS_NUM_THREADS=6 \
      ...
      python -u "$EXP_DIR/run_experiment3.py" --mode smoke --device cuda:0
  } 2>&1 | tee -a "$EXP_DIR/logs/smoke.log"
fi

echo "Experiment 3 started. Logs: $EXP_DIR/logs/repeat_*.log"
PIDS=()
for repeat in $(seq 0 9); do
  ...
  log="$EXP_DIR/logs/repeat_${repeat}.log"
  {
    ...
    CUDA_VISIBLE_DEVICES="$gpu" OMP_NUM_THREADS=6 MKL_NUM_THREADS=6 OPENBLAS_NUM_THREADS=6 \
      PYTHONWARNINGS=ignore PYTHONUNBUFFERED=1 \
      python -u "$EXP_DIR/run_experiment3.py" --mode worker --device cuda:0 --repeat "$repeat"
  } >> "$log" 2>&1 &
  PIDS+=("$!")
done
...
if [[ "$failed" -ne 0 ]]; then
  echo "One or more Experiment 3 workers failed; inspect $EXP_DIR/logs/repeat_*.log" >&2
  exit 1
fi

python -u "$EXP_DIR/run_experiment3.py" --mode aggregate
